 ...
 */
@Component("com.wfzcx.ppms.workflow.component.WFLeaveEventListener")
	...
	public void nodeProcess(String wfid, Map pamam, Map variables,EventSourceVO eventSource) throws Exception {
		...
		if (!"".equals(wfid)) {
			...
			if("".equals(backFlag)){
				...
				String sql ="update "+tableCodeString+" set status='90' where wfid='"+wfid+"'";
				mapDataDao.updateTX(sql);
			}else if ("WITHDRAW".equals(backFlag)) {//撤回
				String sql ="update "+tableCodeString+" set status='99' where wfid='"+wfid+"'";
				mapDataDao.updateTX(sql);
			}
		}
	}
