...
/**
 * @ClassName: FycxService
 * @Description: 复议程序业务处理类
 * @author ybb
 * @date 2017年3月23日 下午3:37:10
 * @version V1.0
 */
@Scope("prototype")
@Service("com.wfzcx.aros.ajbj.service.FycxService")
public class FycxService {
	...
	/**
	 * @Title: queryFycxList
	 * @Description: 案件程序-分页查询案件程序列表
	 * @author ybb
	 * @date 2017年3月23日15:38:51
	 * @param param
	 * @return
	 * @throws AppException
	 */
	public PaginationSupport queryFycxList(Map<String, Object> param) throws AppException {
		...
		sql.append(" and t.protype = '").append(GCC.PROBASEINFO_PROTYPE_XZFYPROGRAM).append("' ");
		...
		sql.append(" and t1.protype ='").append(GCC.PROBASEINFO_PROTYPE_XZFYPROGRAM).append("'");
		sql.append(" ) ");
	...
	/**
	 * @Title: addFycx
	 * @Description: 新增行政复议程序申请
	 * @author ybb
	 * @date 2017年3月23日15:39:10
	 * @param param
	 * @return
	 * @throws Exception
	 */
	...
	public String addFycx(Map<String, Object> param) throws Exception{
		...
		if (casebaseinfo.getNodeid().compareTo(new BigDecimal(GCC.PRONODEBASEINFO_NODEID_END)) == 0) {
			throw new AppException("该案件已审批结束，不能发起程序申请");
		}
		...
		SysUser user = SecureUtil.getCurrentUser();
			...
			//封装案件变更申请信息
		casechangereq.setProtype(GCC.PROBASEINFO_PROTYPE_XZFYPROGRAM);	
		...
		casebaseinfo.setProtype(GCC.PROBASEINFO_PROTYPE_XZFYPROGRAM);
		...
		logSql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYPROGRAM + "'");  
		...
		String agentRemark = StringUtil.stringConvert(param.get("agentRemark"));
			//转化提交操作日志
		probaseinfo = FlowUtil.genSubmittedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYPROGRAM, new BigDecimal(GCC.CASECHANGEREQ_NODEID_ONEAUDIT),
	...
	/**
	 * @Title: fycxAddFlow
	 * @Description: 复议程序发起发送
	 * @author ybb
	 * @date 2017年3月23日15:39:57
	 * @param caseid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fycxAddFlow(String ccrid) throws AppException {
		...
		if(casechangereq == null){
			throw new AppException("未找到对应的复议程序申请");
		}
		...
		if (probaseins == null || probaseins.isEmpty()) {
			throw new AppException("未保存程序发起申请，请保存后再发送");
		}
		...
		probaseinfo = FlowUtil.genProcessedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYPROGRAM, nodeid,
		...
		new_probaseinfo = FlowUtil.genSubmittedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYPROGRAM, newNodeid,
	...
	/**
	 * @Title: updateFycxAudit
	 * @Description: 复议程序审核
	 * @author ybb
	 * @date 2017年3月23日15:40:09
	 * @param param
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void updateFycxAudit(Map<String, Object> param) throws Exception{
		...
		Casechangereq casechangereq = casechangereqDao.get(ccrid);
		if(casechangereq == null){
			throw new AppException("保存失败：未找到对应的复议程序申请");
		}
		...
		sql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYPROGRAM + "'");
		...
		if (probaseinfoList == null || probaseinfoList.isEmpty()) {
			throw new AppException("保存失败：该程序申请未发起流程");
		}
		...
		// 修改过程日志_已接收
		probaseinfo = FlowUtil.genAcceptedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYPROGRAM,
	 ...
	 * @Description: 发送审核流程
	 * @author ybb
	 * @date 2017年3月23日15:40:14
	 * @param ccrid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fycxAuditFlow(String ccrid) throws AppException {
		...
		logSql.append(" from PUB_PROBASEINFO t ");
		logSql.append(" where t.processid = '" + ccrid + "'");
		logSql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYPROGRAM + "'"); // 复议审批
		...
		List<JSONObject> probaseins = mapDataDao.queryListBySQL(logSql.toString());
		if (probaseins == null || probaseins.isEmpty()) {
			throw new AppException("未保存复议程序审核，请保存后再发送");
		}
		...
		SysUser user = SecureUtil.getCurrentUser();
			...
			// 修改流程日志_已处理
		Probaseinfo probaseinfo = probaseinfoDao.get(probaseins.get(0).getString("id"));
		probaseinfo = FlowUtil.genProcessedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYPROGRAM, casechangereq.getNodeid(),
				casechangereq.getCaseid(), probaseinfo, user, null, probaseinfo.getRemark());
		probaseinfoDao.update(probaseinfo);
	...
	/**
	 * @Title: updateFycxDecide
	 * @Description: 保存复议程序决定
	 * @author ybb
	 * @date 2017年3月23日15:40:19
	 * @param param
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void updateFycxDecide(Map<String, Object> param) throws Exception{
	 ...
	 * @Description: 发送决定流程
	 * @author ybb
	 * @date 2017年3月23日15:40:24
	 * @param ccrid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fycxDecideFlow(String ccrid) throws AppException {
		...
		if (GCC.PROBASEINFO_OPTTYPE_PASS.equals(result)) { 	//同意
			...
			casebaseinfo.setNodeid(new BigDecimal(GCC.PRONODEBASEINFO_NODEID_SLUNDERTAKERHEAR));
			casebaseinfo.setCasesort(GCC.CASEBASEINFO_CASESORT_COMMON);
	 ...
	 * @Description: 流程退回
	 * @author ybb
	 * @date 2017年3月23日15:40:47
	 * @param param
	 * @throws Exception
	 */
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fycxReturnFlow(Map<String, Object> param) throws Exception{
		...
		if (null == nodeid_old
			...
			throw new AppException("保存失败：该程序申请所处环节不能退回，请刷新页面再退回");
		}
	...
	/**
	 * @Title: deleteFycxByCcrid
	 * @Description: 根据申请ID删除复议程序申请
	 * @author ybb
	 * @date 2017年3月23日15:40:56
	 * @param param
	 * @throws Exception
	 */
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void deleteFycxByCcrid(Map<String, Object> param) throws Exception{
	...
	public int queryPronodebaseinfoByUserid(){
		...
		sql.append("where protype = '").append(GCC.PROBASEINFO_PROTYPE_XZFYPROGRAM).append("'");
