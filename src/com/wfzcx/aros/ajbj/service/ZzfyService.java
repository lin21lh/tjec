...
/**
 * @ClassName: ZzfyService
 * @Description: 复议终止业务处理类
 * @author ybb
 * @date 2017年3月23日 下午3:58:08
 * @version V1.0
 */
@Scope("prototype")
@Service("com.wfzcx.aros.ajbj.service.ZzfyService")
public class ZzfyService {
	...
	/**
	 * @Title: queryZzfyList
	 * @Description: 案件终止-分页查询案件终止列表
	 * @author ybb
	 * @date 2017年3月23日15:59:45
	 * @param param
	 * @return
	 * @throws AppException
	 */
	public PaginationSupport queryZzfyList(Map<String, Object> param) throws AppException {
		...
		sql.append(" and t.protype = '").append(GCC.PROBASEINFO_PROTYPE_XZFYEND).append("' ");
		...
		sql.append(" and t1.protype ='").append(GCC.PROBASEINFO_PROTYPE_XZFYEND).append("'");
		sql.append(" ) ");
	...
	/**
	 * @Title: addZzfy
	 * @Description: 新增行政复议终止申请
	 * @author ybb
	 * @date 2017年3月23日15:59:58
	 * @param param
	 * @return
	 * @throws Exception
	 */
	...
	public String addZzfy(Map<String, Object> param) throws Exception{
		...
		if (casebaseinfo.getNodeid().compareTo(new BigDecimal(GCC.PRONODEBASEINFO_NODEID_END)) == 0) {
			throw new AppException("该案件已审批结束，不能发起终止申请");
		}
		...
		SysUser user = SecureUtil.getCurrentUser();
			...
			//封装案件变更申请信息
		casechangereq.setProtype(GCC.PROBASEINFO_PROTYPE_XZFYEND);	
		...
		casebaseinfo.setProtype(GCC.PROBASEINFO_PROTYPE_XZFYEND);
		...
		logSql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYEND + "'");  
		...
		String agentRemark = StringUtil.stringConvert(param.get("agentRemark"));
			//转化提交操作日志
		probaseinfo = FlowUtil.genSubmittedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYEND, new BigDecimal(GCC.CASECHANGEREQ_NODEID_ONEAUDIT),
	...
	/**
	 * @Title: zzfyAddFlow
	 * @Description: 复议终止发起发送
	 * @author ybb
	 * @date 2017年3月23日16:00:09
	 * @param caseid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void zzfyAddFlow(String ccrid) throws AppException {
		...
		if(casechangereq == null){
			throw new AppException("未找到对应的复议终止申请");
		}
		...
		if (probaseins == null || probaseins.isEmpty()) {
			throw new AppException("未保存终止发起申请，请保存后再发送");
		}
		...
		probaseinfo = FlowUtil.genProcessedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYEND, nodeid,
		...
		new_probaseinfo = FlowUtil.genSubmittedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYEND, newNodeid,
	...
	/**
	 * @Title: updateZzfyAudit
	 * @Description: 复议终止审核
	 * @author ybb
	 * @date 2017年3月23日16:00:28
	 * @param param
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void updateZzfyAudit(Map<String, Object> param) throws Exception{
		...
		Casechangereq casechangereq = casechangereqDao.get(ccrid);
		if(casechangereq == null){
			throw new AppException("保存失败：未找到对应的复议终止申请");
		}
		...
		sql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYEND + "'");
		...
		if (probaseinfoList == null || probaseinfoList.isEmpty()) {
			throw new AppException("保存失败：该终止申请未发起流程");
		}
		...
		// 修改过程日志_已接收
		probaseinfo = FlowUtil.genAcceptedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYEND,
	 ...
	 * @Description: 发送审核流程
	 * @author ybb
	 * @date 2017年3月23日16:00:34
	 * @param ccrid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void zzfyAuditFlow(String ccrid) throws AppException {
		...
		logSql.append(" from PUB_PROBASEINFO t ");
		logSql.append(" where t.processid = '" + ccrid + "'");
		logSql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYEND + "'"); // 复议审批
		...
		List<JSONObject> probaseins = mapDataDao.queryListBySQL(logSql.toString());
		if (probaseins == null || probaseins.isEmpty()) {
			throw new AppException("未保存复议终止审核，请保存后再发送");
		}
		...
		SysUser user = SecureUtil.getCurrentUser();
			...
			// 修改流程日志_已处理
		Probaseinfo probaseinfo = probaseinfoDao.get(probaseins.get(0).getString("id"));
		probaseinfo = FlowUtil.genProcessedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYEND, casechangereq.getNodeid(),
				casechangereq.getCaseid(), probaseinfo, user, null, probaseinfo.getRemark());
		probaseinfoDao.update(probaseinfo);
	...
	/**
	 * @Title: updateZzfyDecide
	 * @Description: 保存复议终止决定
	 * @author ybb
	 * @date 2017年3月23日16:00:40
	 * @param param
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void updateZzfyDecide(Map<String, Object> param) throws Exception{
	 ...
	 * @Description: 发送决定流程
	 * @author ybb
	 * @date 2017年3月23日16:00:45
	 * @param ccrid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void zzfyDecideFlow(String ccrid) throws AppException {
	 ...
	 * @Description: 流程退回
	 * @author ybb
	 * @date 2017年3月23日16:00:50
	 * @param param
	 * @throws Exception
	 */
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void zzfyReturnFlow(Map<String, Object> param) throws Exception{
		...
		if (null == nodeid_old
			...
			throw new AppException("保存失败：该终止申请所处环节不能退回，请刷新页面再退回");
		}
	...
	/**
	 * @Title: deleteZzfyByCcrid
	 * @Description: 根据申请ID删除复议终止申请
	 * @author ybb
	 * @date 2017年3月23日16:01:01
	 * @param param
	 * @throws Exception
	 */
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void deleteZzfyByCcrid(Map<String, Object> param) throws Exception{
	...
	public int queryPronodebaseinfoByUserid(){
		...
		sql.append("where protype = '").append(GCC.PROBASEINFO_PROTYPE_XZFYEND).append("'");
	...
	public Casechangereq queryFyzzListByCcrid(String ccrid) throws AppException {
		...
		sql.append("t.reqmansay, t.reqedmansay, t.thirdsay, t.ascertain, t.settlement, t.nodeid, t.opttype, t.state, ");
		sql.append("t.remark, t.userid, t.operator, t.opttime, t.region, t.lasttime, t.endreason,  ");
		sql.append(" (select a.name from SYS_YW_DICENUMITEM a where upper(a.elementcode)='PUB_PROBASEINFO_END' and a.status=0 and a.code = t.endreason) endreason, ");
