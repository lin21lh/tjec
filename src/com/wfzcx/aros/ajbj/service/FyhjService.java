...
/**
 * @ClassName: FyhjService
 * @Description: 复议和解业务操作类
 * @author ybb
 * @date 2017年3月23日 下午3:47:23
 * @version V1.0
 */
@Scope("prototype")
@Service("com.wfzcx.aros.ajbj.service.FyhjService")
public class FyhjService {
	...
	/**
	 * @Title: queryFyhjList
	 * @Description: 案件和解-分页查询案件和解列表
	 * @author ybb
	 * @date 2017年3月23日15:48:41
	 * @param param
	 * @return
	 * @throws AppException
	 */
	public PaginationSupport queryFyhjList(Map<String, Object> param) throws AppException {
		...
		sql.append(" and t.protype = '").append(GCC.PROBASEINFO_PROTYPE_XZFYCOMPROMISE).append("' ");
		...
		sql.append(" and t1.protype ='").append(GCC.PROBASEINFO_PROTYPE_XZFYCOMPROMISE).append("'");
		sql.append(" ) ");
	...
	/**
	 * @Title: addFyhj
	 * @Description: 新增行政复议和解申请
	 * @author ybb
	 * @date 2017年3月23日15:48:49
	 * @param param
	 * @return
	 * @throws Exception
	 */
	...
	public String addFyhj(Map<String, Object> param) throws Exception{
		...
		if (casebaseinfo.getNodeid().compareTo(new BigDecimal(GCC.PRONODEBASEINFO_NODEID_END)) == 0) {
			throw new AppException("该案件已审批结束，不能发起和解申请");
		}
		...
		SysUser user = SecureUtil.getCurrentUser();
			...
			//封装案件变更申请信息
		casechangereq.setProtype(GCC.PROBASEINFO_PROTYPE_XZFYCOMPROMISE);	
		...
		casebaseinfo.setProtype(GCC.PROBASEINFO_PROTYPE_XZFYCOMPROMISE);
		...
		logSql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYCOMPROMISE + "'");  
		...
		String agentRemark = StringUtil.stringConvert(param.get("agentRemark"));
			//转化提交操作日志
		probaseinfo = FlowUtil.genSubmittedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYCOMPROMISE, new BigDecimal(GCC.CASECHANGEREQ_NODEID_ONEAUDIT),
	...
	/**
	 * @Title: fyhjAddFlow
	 * @Description: 复议和解发起发送
	 * @author ybb
	 * @date 2017年3月23日15:49:05
	 * @param caseid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyhjAddFlow(String ccrid) throws AppException {
		...
		if(casechangereq == null){
			throw new AppException("未找到对应的复议和解申请");
		}
		...
		if (probaseins == null || probaseins.isEmpty()) {
			throw new AppException("未保存和解发起申请，请保存后再发送");
		}
		...
		probaseinfo = FlowUtil.genProcessedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYCOMPROMISE, nodeid,
		...
		new_probaseinfo = FlowUtil.genSubmittedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYCOMPROMISE, newNodeid,
	...
	/**
	 * @Title: updateFyhjAudit
	 * @Description: 复议和解审核
	 * @author ybb
	 * @date 2017年3月23日15:49:49
	 * @param param
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void updateFyhjAudit(Map<String, Object> param) throws Exception{
		...
		Casechangereq casechangereq = casechangereqDao.get(ccrid);
		if(casechangereq == null){
			throw new AppException("保存失败：未找到对应的复议和解申请");
		}
		...
		sql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYCOMPROMISE + "'");
		...
		if (probaseinfoList == null || probaseinfoList.isEmpty()) {
			throw new AppException("保存失败：该和解申请未发起流程");
		}
		...
		// 修改过程日志_已接收
		probaseinfo = FlowUtil.genAcceptedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYCOMPROMISE,
	 ...
	 * @Description: 发送审核流程
	 * @author ybb
	 * @date 2017年3月23日15:49:57
	 * @param ccrid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyhjAuditFlow(String ccrid) throws AppException {
		...
		logSql.append(" from PUB_PROBASEINFO t ");
		logSql.append(" where t.processid = '" + ccrid + "'");
		logSql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYCOMPROMISE + "'"); // 复议审批
		...
		List<JSONObject> probaseins = mapDataDao.queryListBySQL(logSql.toString());
		if (probaseins == null || probaseins.isEmpty()) {
			throw new AppException("未保存复议和解审核，请保存后再发送");
		}
		...
		SysUser user = SecureUtil.getCurrentUser();
			...
			// 修改流程日志_已处理
		Probaseinfo probaseinfo = probaseinfoDao.get(probaseins.get(0).getString("id"));
		probaseinfo = FlowUtil.genProcessedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYCOMPROMISE, casechangereq.getNodeid(),
				casechangereq.getCaseid(), probaseinfo, user, null, probaseinfo.getRemark());
		probaseinfoDao.update(probaseinfo);
	...
	/**
	 * @Title: updateFyhjDecide
	 * @Description: 保存复议和解决定
	 * @author ybb
	 * @date 2017年3月23日15:50:05
	 * @param param
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void updateFyhjDecide(Map<String, Object> param) throws Exception{
	 ...
	 * @Description: 发送决定流程
	 * @author ybb
	 * @date 2017年3月23日15:50:12
	 * @param ccrid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyhjDecideFlow(String ccrid) throws AppException {
	 ...
	 * @Description: 流程退回
	 * @author ybb
	 * @date 2017年3月23日15:50:28
	 * @param param
	 * @throws Exception
	 */
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyhjReturnFlow(Map<String, Object> param) throws Exception{
		...
		if (null == nodeid_old
			...
			throw new AppException("保存失败：该和解申请所处环节不能退回，请刷新页面再退回");
		}
	...
	/**
	 * @Title: deleteFyhjByCcrid
	 * @Description: 根据申请ID删除复议和解申请
	 * @author ybb
	 * @date 2017年3月23日15:50:39
	 * @param param
	 * @throws Exception
	 */
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void deleteFyhjByCcrid(Map<String, Object> param) throws Exception{
	...
	public int queryPronodebaseinfoByUserid(){
		...
		sql.append("where protype = '").append(GCC.PROBASEINFO_PROTYPE_XZFYCOMPROMISE).append("'");
