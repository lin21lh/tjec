...
/**
 * @ClassName: FyzzService
 * @Description: 复议中止业务处理类
 * @author ybb
 * @date 2017年3月23日 下午3:27:13
 * @version V1.0
 */
@Scope("prototype")
@Service("com.wfzcx.aros.ajbj.service.FyzzService")
public class FyzzService {
	...
	/**
	 * @Title: queryFyzzList
	 * @Description: 案件中止-分页查询案件中止列表
	 * @author ybb
	 * @date 2017年3月22日 上午10:06:06
	 * @param param
	 * @return
	 * @throws AppException
	 */
	public PaginationSupport queryFyzzList(Map<String, Object> param) throws AppException {
	...
	/**
	 * @Title: addFyzz
	 * @Description: 新增行政复议中止申请
	 * @author ybb
	 * @date 2017年3月22日 下午2:47:37
	 * @param param
	 * @return
	 * @throws Exception
	 */
	...
	public String addFyzz(Map<String, Object> param) throws Exception{
		...
		if (casebaseinfo.getNodeid().compareTo(new BigDecimal(GCC.PRONODEBASEINFO_NODEID_END)) == 0) {
			throw new AppException("该案件已审批结束，不能发起中止申请");
		}
		...
		SysUser user = SecureUtil.getCurrentUser();
			...
			//封装案件变更申请信息
		casechangereq.setProtype(GCC.PROBASEINFO_PROTYPE_XZFYSUSPEND);	
		...
		logSql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYSUSPEND + "'");  
		...
		String agentRemark = StringUtil.stringConvert(param.get("agentRemark"));
			//转化提交操作日志
		probaseinfo = FlowUtil.genSubmittedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYSUSPEND, new BigDecimal(GCC.CASECHANGEREQ_NODEID_ONEAUDIT),
	...
	/**
	 * @Title: fyzzAddFlow
	 * @Description: 复议中止发起发送
	 * @author ybb
	 * @date 2017年3月22日 下午4:00:49
	 * @param caseid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyzzAddFlow(String ccrid) throws AppException {
		...
		if(casechangereq == null){
			throw new AppException("未找到对应的复议中止申请");
		}
		...
		if (probaseins == null || probaseins.isEmpty()) {
			throw new AppException("未保存中止发起申请，请保存后再发送");
		}
		...
		probaseinfo = FlowUtil.genProcessedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYSUSPEND, nodeid,
		...
		new_probaseinfo = FlowUtil.genSubmittedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYSUSPEND, newNodeid,
	...
	/**
	 * @Title: updateFyzzAudit
	 * @Description: 复议中止审核
	 * @author ybb
	 * @date 2017年3月22日 下午6:40:05
	 * @param param
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void updateFyzzAudit(Map<String, Object> param) throws Exception{
		...
		Casechangereq casechangereq = casechangereqDao.get(ccrid);
		if(casechangereq == null){
			throw new AppException("保存失败：未找到对应的复议中止申请");
		}
		...
		sql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYSUSPEND + "'");
		...
		if (probaseinfoList == null || probaseinfoList.isEmpty()) {
			throw new AppException("保存失败：该中止申请未发起流程");
		}
		...
		// 修改过程日志_已接收
		probaseinfo = FlowUtil.genAcceptedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYSUSPEND,
	 ...
	 * @Description: 发送审核流程
	 * @author ybb
	 * @date 2017年3月22日 下午6:53:54
	 * @param ccrid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyzzAuditFlow(String ccrid) throws AppException {
		...
		logSql.append(" from PUB_PROBASEINFO t ");
		logSql.append(" where t.processid = '" + ccrid + "'");
		logSql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYSUSPEND + "'"); // 复议审批
		...
		List<JSONObject> probaseins = mapDataDao.queryListBySQL(logSql.toString());
		if (probaseins == null || probaseins.isEmpty()) {
			throw new AppException("未保存复议中止审核，请保存后再发送");
		}
		...
		SysUser user = SecureUtil.getCurrentUser();
			...
			// 修改流程日志_已处理
		Probaseinfo probaseinfo = probaseinfoDao.get(probaseins.get(0).getString("id"));
		probaseinfo = FlowUtil.genProcessedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYSUSPEND, casechangereq.getNodeid(),
				casechangereq.getCaseid(), probaseinfo, user, null, probaseinfo.getRemark());
		probaseinfoDao.update(probaseinfo);
	...
	/**
	 * @Title: updateFyzzDecide
	 * @Description: 保存复议中止决定
	 * @author ybb
	 * @date 2017年3月22日 下午7:15:37
	 * @param param
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void updateFyzzDecide(Map<String, Object> param) throws Exception{
	 ...
	 * @Description: 发送决定流程
	 * @author ybb
	 * @date 2017年3月22日 下午7:25:41
	 * @param ccrid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyzzDecideFlow(String ccrid) throws AppException {
	 ...
	 * @Description: 流程退回
	 * @author ybb
	 * @date 2017年3月22日 下午9:13:35
	 * @param param
	 * @throws Exception
	 */
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyzzReturnFlow(Map<String, Object> param) throws Exception{
		...
		if (null == nodeid_old
			...
			throw new AppException("保存失败：该中止申请所处环节不能退回，请刷新页面再退回");
		}
	...
	/**
	 * @Title: deleteFyzzByCcrid
	 * @Description: 根据申请ID删除复议中止申请
	 * @author ybb
	 * @date 2017年3月23日 下午3:01:11
	 * @param param
	 * @throws Exception
	 */
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void deleteFyzzByCcrid(Map<String, Object> param) throws Exception{
	...
	public int queryPronodebaseinfoByUserid(){
		...
		sql.append("where protype = '").append(GCC.PROBASEINFO_PROTYPE_XZFYSUSPEND).append("'");
