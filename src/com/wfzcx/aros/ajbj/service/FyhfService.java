...
/**
 * @ClassName: FyhfService
 * @Description: 复议恢复业务处理类
 * @author ybb
 * @date 2017年3月23日 下午3:41:32
 * @version V1.0
 */
@Scope("prototype")
@Service("com.wfzcx.aros.ajbj.service.FyhfService")
public class FyhfService {
	...
	/**
	 * @Title: queryFyhfList
	 * @Description: 案件恢复-分页查询案件恢复列表
	 * @author ybb
	 * @date 2017年3月23日15:44:02
	 * @param param
	 * @return
	 * @throws AppException
	 */
	public PaginationSupport queryFyhfList(Map<String, Object> param) throws AppException {
		...
		sql.append(" and t.protype = '").append(GCC.PROBASEINFO_PROTYPE_XZFYRECOVER).append("' ");
		...
		sql.append(" and t1.protype ='").append(GCC.PROBASEINFO_PROTYPE_XZFYRECOVER).append("'");
		sql.append(" ) ");
	...
	/**
	 * @Title: addFyhf
	 * @Description: 新增行政复议恢复申请
	 * @author ybb
	 * @date 2017年3月23日15:44:11
	 * @param param
	 * @return
	 * @throws Exception
	 */
	...
	public String addFyhf(Map<String, Object> param) throws Exception{
		...
		if (casebaseinfo.getNodeid().compareTo(new BigDecimal(GCC.PRONODEBASEINFO_NODEID_END)) == 0) {
			throw new AppException("该案件已审批结束，不能发起恢复申请");
		}
		...
		SysUser user = SecureUtil.getCurrentUser();
			...
			//封装案件变更申请信息
		casechangereq.setProtype(GCC.PROBASEINFO_PROTYPE_XZFYRECOVER);	
		...
		casebaseinfo.setProtype(GCC.PROBASEINFO_PROTYPE_XZFYRECOVER);
		...
		logSql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYRECOVER + "'");  
		...
		String agentRemark = StringUtil.stringConvert(param.get("agentRemark"));
			//转化提交操作日志
		probaseinfo = FlowUtil.genSubmittedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYRECOVER, new BigDecimal(GCC.CASECHANGEREQ_NODEID_ONEAUDIT),
	...
	/**
	 * @Title: fyhfAddFlow
	 * @Description: 复议恢复发起发送
	 * @author ybb
	 * @date 2017年3月23日15:45:07
	 * @param caseid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyhfAddFlow(String ccrid) throws AppException {
		...
		if(casechangereq == null){
			throw new AppException("未找到对应的复议恢复申请");
		}
		...
		if (probaseins == null || probaseins.isEmpty()) {
			throw new AppException("未保存恢复发起申请，请保存后再发送");
		}
		...
		probaseinfo = FlowUtil.genProcessedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYRECOVER, nodeid,
		...
		new_probaseinfo = FlowUtil.genSubmittedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYRECOVER, newNodeid,
	...
	/**
	 * @Title: updateFyhfAudit
	 * @Description: 复议恢复审核
	 * @author ybb
	 * @date 2017年3月23日15:45:18
	 * @param param
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void updateFyhfAudit(Map<String, Object> param) throws Exception{
		...
		Casechangereq casechangereq = casechangereqDao.get(ccrid);
		if(casechangereq == null){
			throw new AppException("保存失败：未找到对应的复议恢复申请");
		}
		...
		sql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYRECOVER + "'");
		...
		if (probaseinfoList == null || probaseinfoList.isEmpty()) {
			throw new AppException("保存失败：该恢复申请未发起流程");
		}
		...
		// 修改过程日志_已接收
		probaseinfo = FlowUtil.genAcceptedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYRECOVER,
	 ...
	 * @Description: 发送审核流程
	 * @author ybb
	 * @date 2017年3月23日15:45:24
	 * @param ccrid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyhfAuditFlow(String ccrid) throws AppException {
		...
		logSql.append(" from PUB_PROBASEINFO t ");
		logSql.append(" where t.processid = '" + ccrid + "'");
		logSql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYRECOVER + "'"); // 复议审批
		...
		List<JSONObject> probaseins = mapDataDao.queryListBySQL(logSql.toString());
		if (probaseins == null || probaseins.isEmpty()) {
			throw new AppException("未保存复议恢复审核，请保存后再发送");
		}
		...
		SysUser user = SecureUtil.getCurrentUser();
			...
			// 修改流程日志_已处理
		Probaseinfo probaseinfo = probaseinfoDao.get(probaseins.get(0).getString("id"));
		probaseinfo = FlowUtil.genProcessedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYRECOVER, casechangereq.getNodeid(),
				casechangereq.getCaseid(), probaseinfo, user, null, probaseinfo.getRemark());
		probaseinfoDao.update(probaseinfo);
	...
	/**
	 * @Title: updateFyhfDecide
	 * @Description: 保存复议恢复决定
	 * @author ybb
	 * @date 2017年3月23日15:45:29
	 * @param param
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void updateFyhfDecide(Map<String, Object> param) throws Exception{
	 ...
	 * @Description: 发送决定流程
	 * @author ybb
	 * @date 2017年3月23日15:45:34
	 * @param ccrid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyhfDecideFlow(String ccrid) throws AppException {
	 ...
	 * @Description: 流程退回
	 * @author ybb
	 * @date 2017年3月23日15:45:40
	 * @param param
	 * @throws Exception
	 */
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyhfReturnFlow(Map<String, Object> param) throws Exception{
		...
		if (null == nodeid_old
			...
			throw new AppException("保存失败：该恢复申请所处环节不能退回，请刷新页面再退回");
		}
	...
	public PaginationSupport queryCaseList(Map<String, Object> param) throws AppException {
		...
		sql.append("t.suspenddate, t.restorereason, t.delayreason, t.delaydate, t.withdrawreason, t.reqmanrequest, ");
		sql.append("t.reqmansay, t.reqedmansay, t.thirdsay, t.ascertain, t.settlement, t.nodeid, t.opttype, t.state, ");
		sql.append("t.remark, t.userid, t.operator, t.opttime, t.region, t.lasttime,  ");
		sql.append(" (select a.name from SYS_YW_DICENUMITEM a where upper(a.elementcode)='PUB_PROBASEINFO_SUSPEND' and a.status=0 and a.code = t.suspendreason) suspendreason_mc ");
		sql.append("from B_CASECHANGEREQ t, B_CASEBASEINFO t1 ");
		sql.append("where t.caseid = t1.caseid ");
		sql.append("and t1.protype = '").append(GCC.PROBASEINFO_PROTYPE_XZFYSUSPEND).append("'");
		
		sql.append("and t.protype = '").append(GCC.PROBASEINFO_PROTYPE_XZFYSUSPEND).append("'");
		sql.append("and t.state = '").append(GCC.CASECHANGEREQ_STATE_COMPLETE).append("'");
	...
	/**
	 * @Title: deleteFyhfByCcrid
	 * @Description: 根据申请ID删除复议恢复申请
	 * @author ybb
	 * @date 2017年3月23日15:45:49
	 * @param param
	 * @throws Exception
	 */
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void deleteFyhfByCcrid(Map<String, Object> param) throws Exception{
	...
	public int queryPronodebaseinfoByUserid(){
		...
		sql.append("where protype = '").append(GCC.PROBASEINFO_PROTYPE_XZFYRECOVER).append("'");
	...
	public Casechangereq queryFyzzListByCcrid(String ccrid) throws AppException {
		...
		sql.append("t.reqmansay, t.reqedmansay, t.thirdsay, t.ascertain, t.settlement, t.nodeid, t.opttype, t.state, ");
		sql.append("t.remark, t.userid, t.operator, t.opttime, t.region, t.lasttime, t.endreason,  ");
		sql.append(" (select a.name from SYS_YW_DICENUMITEM a where upper(a.elementcode)='PUB_PROBASEINFO_SUSPEND' and a.status=0 and a.code = t.suspendreason) suspendreason, ");
