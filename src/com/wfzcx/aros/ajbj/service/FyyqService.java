...
/**
 * @ClassName: FyyqService
 * @Description: 复议延期业务操作类
 * @author ybb
 * @date 2017年3月23日 下午3:54:38
 * @version V1.0
 */
@Scope("prototype")
@Service("com.wfzcx.aros.ajbj.service.FyyqService")
public class FyyqService {
	...
	/**
	 * @Title: queryFyyqList
	 * @Description: 案件延期-分页查询案件延期列表
	 * @author ybb
	 * @date 2017年3月23日15:56:36
	 * @param param
	 * @return
	 * @throws AppException
	 */
	public PaginationSupport queryFyyqList(Map<String, Object> param) throws AppException {
		...
		sql.append(" and t.protype = '").append(GCC.PROBASEINFO_PROTYPE_XZFYDELAY).append("' ");
		...
		sql.append(" and t1.protype ='").append(GCC.PROBASEINFO_PROTYPE_XZFYDELAY).append("'");
		sql.append(" ) ");
	...
	/**
	 * @Title: addFyyq
	 * @Description: 新增行政复议延期申请
	 * @author ybb
	 * @date 2017年3月23日15:56:49
	 * @param param
	 * @return
	 * @throws Exception
	 */
	...
	public String addFyyq(Map<String, Object> param) throws Exception{
		...
		if (casebaseinfo.getNodeid().compareTo(new BigDecimal(GCC.PRONODEBASEINFO_NODEID_END)) == 0) {
			throw new AppException("该案件已审批结束，不能发起延期申请");
		}
		...
		SysUser user = SecureUtil.getCurrentUser();
			...
			//封装案件变更申请信息
		casechangereq.setProtype(GCC.PROBASEINFO_PROTYPE_XZFYDELAY);	
		...
		casebaseinfo.setProtype(GCC.PROBASEINFO_PROTYPE_XZFYDELAY);
		...
		logSql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYDELAY + "'");  
		...
		String agentRemark = StringUtil.stringConvert(param.get("agentRemark"));
			//转化提交操作日志
		probaseinfo = FlowUtil.genSubmittedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYDELAY, new BigDecimal(GCC.CASECHANGEREQ_NODEID_ONEAUDIT),
	...
	/**
	 * @Title: fyyqAddFlow
	 * @Description: 复议延期发起发送
	 * @author ybb
	 * @date 2017年3月23日15:57:17
	 * @param caseid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyyqAddFlow(String ccrid) throws AppException {
		...
		if(casechangereq == null){
			throw new AppException("未找到对应的复议延期申请");
		}
		...
		if (probaseins == null || probaseins.isEmpty()) {
			throw new AppException("未保存延期发起申请，请保存后再发送");
		}
		...
		probaseinfo = FlowUtil.genProcessedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYDELAY, nodeid,
		...
		new_probaseinfo = FlowUtil.genSubmittedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYDELAY, newNodeid,
	...
	/**
	 * @Title: updateFyyqAudit
	 * @Description: 复议延期审核
	 * @author ybb
	 * @date 2017年3月23日15:57:22
	 * @param param
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void updateFyyqAudit(Map<String, Object> param) throws Exception{
		...
		Casechangereq casechangereq = casechangereqDao.get(ccrid);
		if(casechangereq == null){
			throw new AppException("保存失败：未找到对应的复议延期申请");
		}
		...
		sql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYDELAY + "'");
		...
		if (probaseinfoList == null || probaseinfoList.isEmpty()) {
			throw new AppException("保存失败：该延期申请未发起流程");
		}
		...
		// 修改过程日志_已接收
		probaseinfo = FlowUtil.genAcceptedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYDELAY,
	 ...
	 * @Description: 发送审核流程
	 * @author ybb
	 * @date 2017年3月23日15:57:27
	 * @param ccrid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyyqAuditFlow(String ccrid) throws AppException {
		...
		logSql.append(" from PUB_PROBASEINFO t ");
		logSql.append(" where t.processid = '" + ccrid + "'");
		logSql.append(" and t.protype = '" + GCC.PROBASEINFO_PROTYPE_XZFYDELAY + "'"); // 复议审批
		...
		List<JSONObject> probaseins = mapDataDao.queryListBySQL(logSql.toString());
		if (probaseins == null || probaseins.isEmpty()) {
			throw new AppException("未保存复议延期审核，请保存后再发送");
		}
		...
		SysUser user = SecureUtil.getCurrentUser();
			...
			// 修改流程日志_已处理
		Probaseinfo probaseinfo = probaseinfoDao.get(probaseins.get(0).getString("id"));
		probaseinfo = FlowUtil.genProcessedOperationData(GCC.PROBASEINFO_PROTYPE_XZFYDELAY, casechangereq.getNodeid(),
				casechangereq.getCaseid(), probaseinfo, user, null, probaseinfo.getRemark());
		probaseinfoDao.update(probaseinfo);
	...
	/**
	 * @Title: updateFyyqDecide
	 * @Description: 保存复议延期决定
	 * @author ybb
	 * @date 2017年3月23日15:57:33
	 * @param param
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void updateFyyqDecide(Map<String, Object> param) throws Exception{
	 ...
	 * @Description: 发送决定流程
	 * @author ybb
	 * @date 2017年3月23日15:57:39
	 * @param ccrid
	 * @throws AppException
	 */
	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyyqDecideFlow(String ccrid) throws AppException {
	 ...
	 * @Description: 流程退回
	 * @author ybb
	 * @date 2017年3月23日15:57:45
	 * @param param
	 * @throws Exception
	 */
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void fyyqReturnFlow(Map<String, Object> param) throws Exception{
		...
		if (null == nodeid_old
			...
			throw new AppException("保存失败：该延期申请所处环节不能退回，请刷新页面再退回");
		}
	...
	/**
	 * @Title: deleteFyyqByCcrid
	 * @Description: 根据申请ID删除复议延期申请
	 * @author ybb
	 * @date 2017年3月23日15:57:55
	 * @param param
	 * @throws Exception
	 */
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void deleteFyyqByCcrid(Map<String, Object> param) throws Exception{
	...
	public int queryPronodebaseinfoByUserid(){
		...
		sql.append("where protype = '").append(GCC.PROBASEINFO_PROTYPE_XZFYDELAY).append("'");
