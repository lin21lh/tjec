...
public class JpdlModelDrawer {
	...
	public BufferedImage draw(JpdlModel jpdlModel) throws IOException {
		...
		Font font;
		try {
			System.out.println("尝试加载字体：微软雅黑...");
			font = new Font("微软雅黑", Font.PLAIN, DEFAULT_FONT_SIZE);

		} catch (Exception e) {

			e.printStackTrace();
			font = new Font("宋体", Font.PLAIN, DEFAULT_FONT_SIZE);
		}
		g2.setFont(font);
	...
	private void drawNode(Map<String, Node> nodes, Graphics2D g2, Font font)
		...
		for (Node node : nodes.values()) {
			...
			if (nodeInfos.get(node.getType()) != null) {
				...
				int xLabel = (int) (node.getX() + ((node.getWitdth() - r2
						.getWidth()) / 2));
