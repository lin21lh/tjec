/************************************************************
 * 类名：SysFileManageServiceImpl.java
 *
 * 类别：Service实现类
 * 功能：附件管理服务实现类
 * 
...
@Scope("prototype")
@Service
public class SysFileManageServiceImpl implements SysFileManageService {

	String savePath = GCC.SYS_FILE_SAVEPATH;
	...
	public EasyUITotalResult query(String keyid, String elementcode, Integer page, Integer rows) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		return EasyUITotalResult.from(fileManageDao.query(keyid, elementcode, page, rows));
	}
	...
	public List<Map<String, Object>> queryFiles(String elementcode, String keyid, String stepid,String showFileLength) {
		...
		for (SysFileManage file : files) {
			try {
				//是否显示缩略的字段
				if(showFileLength !=null && !"".equals(showFileLength)){
	...
	public List<Map<String, Object>> queryFilesByItemid(String itemid,String showFileLength) {
		...
		List<SysFileManage> files = fileManageComponent.findFilesByItemid(itemid);
		...
		for (SysFileManage file : files) {
			try {
				...
				if(showFileLength !=null && !"".equals(showFileLength)){
					int fileLength = Integer.parseInt(showFileLength);
					String fileName = file.getOriginalfilename();
					if(fileName.getBytes().length>fileLength){
						file.setOmitFileName(StringUtil.subStr(fileName,fileLength)+"...");
					}else{
						file.setOmitFileName(file.getOriginalfilename());
					}
				}
...
//			fileManage.setSavemode(Integer.valueOf(upLoadFile.getmap("savemode")));
			...
			if(SecureUtil.getCurrentUser() == null){
				fileManage.setUsercode(sysUserDao.get(Long.valueOf(upLoadFile.getmap("userid"))).getUsercode());
			} else {
				fileManage.setUsercode(SecureUtil.getCurrentUser().getUsercode());
			}
