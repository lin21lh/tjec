...
public class NumberUtil {
	 ...
	 * @return
	 */
	public static String randomNum(char[] arrayChar,int length){
		if(arrayChar.length<length){
		    throw new NoSuchElementException("数组的length必须大于length");
		}
		Random random=new Random();
		LinkedList<Short> list=new LinkedList<Short>();
		for (int i = 0; i < arrayChar.length; i++) {
			list.add((short)arrayChar[i]);
		}
		char string[]=new char[length];
		for (int i = 0; i < length; i++) {
			int index=random.nextInt(list.size());
			string[i]=(char) list.remove(index).shortValue();
		}
	...
	public static final int ten_8_pow=100000000;
	 ...
	 public static double round(double number,int scale){
		 ...
		 switch (scale){
			case 2:
				multiple=ten_2_pow; break;
			case 6:
				multiple=ten_6_pow; break;
			case 8:
				multiple=ten_8_pow; break;
			default:
	 ...
	 public static BigDecimal bigDecimal(Object o) {
				...
				if (o instanceof BigDecimal)
					...
					demLval = new BigDecimal(FormatUtil.fromDouble_6(((Float)o).doubleValue()));
	 ...
	 /**
		...
		public static  <T extends Number>T doubleToNubmer(double number,Class<T> clazz){
			if(clazz==Double.class){
				...
				return (T) ConvertUtils.convert(FormatUtil.fromDouble_6(number), clazz);
