...
public class Condition{
	...
	public Result getResult() {
		...
		return new Condition.Result(values, sql.toString());
	}
	...
	public void add(String sql ) {
		Entry newObj = new Condition.Entry(noneValue, sql);
		if (size == 0) {
			first = newObj;
		} else {
			last.next = newObj;
	...
	public void add(String sql,Object value ) {
		Entry newObj = new Condition.Entry(value, sql);
		if (size == 0) {
			first = newObj;
		} else {
			last.next = newObj;
