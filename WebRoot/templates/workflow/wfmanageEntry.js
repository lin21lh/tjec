...
var _debug_flag="on";  // 显示设计器工具栏调试工具 on打开 off 关闭
$(function() {
	$('#wftree').tree(
			{
				url : urls.wftree,
...
if(_debug_flag!='on'){
	$('#linkbutton_printETcache').hide();
	$('#linkbutton_printATcache').hide();
	$('#linkbutton_parse').hide();
	$('#linkbutton_view_jpdl').hide();
...
function showDetail(node) {
	...
	case 'leaf':
		...
		var v_=node.text;
		var p=$('#wftree').tree('getParent',node.target);
		$.post(urls.getWorkflowProcversion,{
			...
			// 初始化流程设计器
			getWorkflowDesigner().init("#widgets","#canvas","#prop",null,"UPDATE");
			
			getWorkflowDesigner().processInfo.startdate=obj.startdate;
			getWorkflowDesigner().processInfo.enddate=obj.enddate;
			getWorkflowDesigner().processInfo.firstnode=obj.firstnode;
			// 加载版本
			getWorkflowDesigner().loadWorkflowVersion(node.id);
...
function saveAndDeploy(){
	var  processInfo=getWorkflowDesigner().processInfo;
	if(!processInfo.name){
...
//	
//	var wd=getWorkflowDesigner();
//	var actiArray=wd.actiArray;
...
// +name+'" xmlns="http://jbpm.org/4.4/jpdl" key="'+key+'"
// version="'+maxVersion+'"></process>';
// $('#jpdl').val(initXml);
// parseJpdl();
	
	// 初始化流程设计器
	getWorkflowDesigner().init("#widgets","#canvas","#prop",null,"NEW");
	// 加载版本
	getWorkflowDesigner().createNewWorkflowVersion(key,maxVersion,node.text);
...
function viewJpdl() {
	getWorkflowDesigner().generateJpdl();
	document.getElementById("jpdl").value = getWorkflowDesigner().getJpdl();
	$('#xml_window').window('open');
}

function printExtAttrCache(){
	console.log("扩展属性缓存如下：");
	console.log(getWorkflowDesigner().extAttrCache);
}

function printActiArray(){
	console.log("流程图节点缓存如下：");
	console.log(getWorkflowDesigner().actiArray);
}