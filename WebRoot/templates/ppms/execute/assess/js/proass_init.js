 ...
 * proass_init.js
 * */
var baseUrl = contextpath + "execute/controller/ProjectAssessController/";
//路径
var urls = {
	qryProAss : baseUrl + "qryProAss.do",
	delProAss : baseUrl+ "delProAss.do",
	optProAssView : baseUrl+ "optProAssView.do",
	saveProAss : baseUrl+ "saveProAss.do",
	sendWFUrl : baseUrl + "sendWorkFlow.do",
	revokeWFUrl : baseUrl + "revokeWorkFlow.do",
	qryProAssess : baseUrl +"qryProAssess.do",
	delProAssess : baseUrl +"delProAssess.do"
...
function loaddatagrid(){
	datagrid_proass = $("#datagrid_proass").datagrid({
		fit : true,
		stripe : true,
		singleSelect : true,
		rownumbers : true,
		...
		, {field : "createuser",title : "创建人",halign : 'center',fixed : true,	width : 120,sortable:true,hidden:true,hidden:true}
...
function qryProAss(){
	...
	$("#datagrid_proass").datagrid("load",param);
}
 ...
 */
function addProAss(){
	var rows = datagrid_proass.datagrid("getSelections");
	if (rows.length != 1) {
	...
	showModalDialog(500, "proass_form", types.add, datagrid_proass, "中期评估新增",urls.optProAssView+"?optFlag="+types.add, urls.saveProAss+"?optFlag="+types.add);
}
...
function editProAss(){
	...
	showModalDialog(500, "proass_form", types.edit, datagrid_proass, "中期评估修改",urls.optProAssView+"?optFlag="+types.edit, urls.saveProAss+"?optFlag="+types.edit);
}
...
function detProAss(){
	...
	showModalDialog(500, "proass_form", types.view, datagrid_proass, "中期评估详情",urls.optProAssView+"?optFlag="+types.view, urls.saveProAss+"?optFlag="+types.view);
}
...
function proPerformance(mdDialog,performanceid,projectid){
	var grid = mdDialog.find("#advance_organ").datagrid({
        height: 360,
        url : urls.qryProAssess,
        queryParams : {performanceid:performanceid,projectid:projectid},
        rownumbers : true,
        singleSelect: true,
        idField: 'assessid',
        columns: [[//显示的列
                   { field: 'assessid', title: '主键id', halign : 'center',width: 100, sortable: true,hidden:true }
                   ,{ field: 'projectid', title: '项目id', halign : 'center',width: 200 ,hidden:true}
                   ,{ field: 'assessTime', title: '评估时间', halign : 'center',width: 220 ,editor: { type: 'datebox',options: { required: true,missingMessage:'请输入评估时间',editable:false}}}
                   ,{ field: 'assessContent', title: '评估说明',halign : 'center', width: 500,sortable: true,editor: { type: 'validatebox',options: { required: true,missingMessage:'请输入评估说明',validType:{length:[0,500]}}}}
                   ,{ field: 'assessPath', title: '附件', halign : 'center',width: 100,sortable: true ,hidden:true}
                   ]],
                   toolbar: [{
                	   id:"fin_add",
                       ...
                       text: '删除', iconCls: 'icon-remove', handler: function () {
                           ...
                           if(row ==''){
                           	...
                           	parent.$.messager.confirm("删除确认", "确认删除选中的数据？", function (r) {
                                    if (r) {
                                    	if(row[0].assessid==undefined){//没有录数据的情况下，直接在界面删除
                                       		...
                                       		$.post(urls.delProAssess, {
                	                       		assessid :row[0].assessid
                	            			}, function(result) {
	...
	form.form("submit",{
			...
			success : function(result) {
				...
				if (result.success) {
					...
					datagrid_proass.datagrid("reload");
				} else {
