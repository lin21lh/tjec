...
var urls = {
	...
	hasRevokelist : baseUrl + "queryAllAccount.do",
	addRevokeInfo : baseUrl + "saveAddRevokeInfoNoWF.do",
	editRevokeInfo : baseUrl + "saveEditRevokeInfoNoWF.do",
...
$(function() {
	...
	$("#dealStatus").combobox({
		...
		value : "1",
		editable : false,
		data : [ {text : "待处理", value : "1"}, {text : "已注销", value : "2"}],
...
function loadHashRevokeSearch(){
	...
	comboboxFuncByCondFilter(menuid,"changetype", "REVOKETYPE", "code", "name");//注销事项
...
function addRevokeInfo() {
	...
	showModalDialog(485, "revoke_add", types.add, datagridNoRevoke, "账户注销",
...
function editRevokeInfo() {
	...
	showModalDialog(485, "revoke_add", types.edit, datagridHasRevoke, "账户修改",
...
function queryRevokeInfo() {
	...
	showModalDialog(485, "revoke_add", types.view, datagridHasRevoke, "账户详情",
	...
	if (operType == types.view) {
		...
		buttons = [ {
			...
			handler : function() {
				...
				submitForm(url+"?isSub=0", form,operType,"0");
			}
		},{
			text : "提交",
			iconCls : "icon-save",
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;
				submitForm(url+"?isSub=1", form,operType,"1");
			}
		},{
			text : "附件管理",
			iconCls : "icon-files",
 ...
 *            表单url
 */
function submitForm(url, form,operType,isSub) {
	var form = parent.$.modalDialog.handler.find('#' + form);
	var isValid = form.form('validate');
	if (!isValid) {
		return;
	}
	var message = "确定要注销该账户？";
	if(isSub == "0"){
		message = "确定要保存？";
	}
	parent.$.messager.confirm("注销确认", message, function(r) {
		if (r) {
			form.form("submit",	{
				url : url,
				onSubmit : function() {
