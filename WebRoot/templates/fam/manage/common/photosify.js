...
$(function() {
	...
	var length = parent.$("#"+tdId).children("div").length;//获取父页面的div个数，每个div代表一个附件
	var divs = parent.$("#"+tdId).children("div");
	...
	$("#uploadify").uploadify({
        ...
        onUploadSuccess : function(file, data, response) {
        	...
        	//验证是否存在同名文件
        	var l = parent.$("#"+tdId).children("div").length;
        	var flag=false;
        	for(var i=0;i<l;i++){
        		//var hasFileName=tableControl.find("tr").eq(i).children("td").eq(0).find("a").text();
        		var hasFileName=parent.$("#"+tdId).children("div").eq(i).attr("fileName");
