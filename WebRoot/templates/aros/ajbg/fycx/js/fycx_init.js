//请求路径
var baseUrl = contextpath + "aros/ajbg/fycx/FycxController/";
var urls = {
	// 查询复议程序案件列表
	gridUrl:baseUrl + "queryFycxList.do",
	// 复议程序发起初始化
	fycxAddInitUrl:baseUrl + "fycxAddInit.do",
	// 复议程序发起保存
	fycxAddSaveUrl:baseUrl + "fycxAddSave.do",
	// 复议程序发起发送
	fycxAddSendUrl:baseUrl + "fycxAddFlow.do",
	// 复议程序审批编辑
	fycxAuditInitUrl:baseUrl + "fycxAuditInit.do",
	// 复议程序审批保存
	fycxAuditSaveUrl:baseUrl + "fycxAuditSave.do",
	// 复议程序审批发送
	fycxAuditSendUrl:baseUrl + "fycxAuditFlow.do",
	// 处理决定编辑
	fycxDecideInitUrl:baseUrl + "fycxDecideInit.do",
	// 处理决定保存
	fycxDecideSaveUrl:baseUrl + "fycxDecideSave.do",
	// 处理决定发送
	fycxDecideSendUrl:baseUrl + "fycxDecideFlow.do",
	// 回退
	backFlowPageUrl:baseUrl + "backFlowPage.do",
	// 回退
	returnUrl:baseUrl + "fycxReturnFlow.do",
	// 删除
	deleteUrl:baseUrl + "fycxDelete.do"
};
 ...
 * 程序发起
 */
function fycxAdd(title, code, ccrid, nodeid) {
	parent.$.modalDialog({
		title: "复议程序发起",
		width: 900,
		height: 550,
		href: urls.fycxAddInitUrl,
		queryParams: {
			ccrid: ccrid,
			nodeid: 10 
		},
		...
		buttons:[{
				...
				iconCls:"icon-save",
				handler:function() {
					submitForm(urls.fycxAddSaveUrl, "fycxAddForm", 10);
				}
			},{
				id: "sendBtn",
				disabled:true,
				...
				iconCls:"icon-redo",
				handler:function() {
					send(urls.fycxAddSendUrl, ccrid, 10);
				}
			},{
				text:"关闭",
				iconCls:"icon-cancel",
...
function xzfyEditDialog(row) {
	...
	if (10 == nodeidtmp) {
		xzfyEdit("复议程序发起", 900, 550, urls.fycxAddInitUrl, row, "fycxAddForm", urls.fycxAddSaveUrl, urls.fycxAddSendUrl);
	}
	else if (20 == nodeidtmp || 30 == nodeidtmp || 40 == nodeidtmp) {
		xzfyEdit("复议程序审批", 900, 550, urls.fycxAuditInitUrl, row, "fycxAuditForm", urls.fycxAuditSaveUrl, urls.fycxAuditSendUrl);
	}
	else if (50 == nodeidtmp) {
		xzfyEdit("复议程序决定", 900, 550, urls.fycxDecideInitUrl, row, "fycxDecideForm", urls.fycxDecideSaveUrl, urls.fycxDecideSendUrl);
	}
}
...
function isReadonlyByNodeid(nodeid){
	...
	var form = parent.$.modalDialog.handler.find('#fycxAuditForm');
	if (20 == nodeid) { // 科室
