...
var urls = {
	// 查询复议恢复案件列表
	gridUrl:baseUrl + "queryFyhfList.do",
	// 复议恢复发起初始化
	fyhfAddInitUrl:baseUrl + "fyhfAddInit.do",
	// 复议恢复发起保存
	fyhfAddSaveUrl:baseUrl + "fyhfAddSave.do",
	// 复议恢复发起发送
	fyhfAddSendUrl:baseUrl + "fyhfAddFlow.do",
	// 复议恢复审批编辑
	fyhfAuditInitUrl:baseUrl + "fyhfAuditInit.do",
	// 复议恢复审批保存
	fyhfAuditSaveUrl:baseUrl + "fyhfAuditSave.do",
	// 复议恢复审批发送
	fyhfAuditSendUrl:baseUrl + "fyhfAuditFlow.do",
	// 处理决定编辑
	fyhfDecideInitUrl:baseUrl + "fyhfDecideInit.do",
	// 处理决定保存
	fyhfDecideSaveUrl:baseUrl + "fyhfDecideSave.do",
	// 处理决定发送
	fyhfDecideSendUrl:baseUrl + "fyhfDecideFlow.do",
	// 回退
	backFlowPageUrl:baseUrl + "backFlowPage.do",
	// 回退
	returnUrl:baseUrl + "fyhfReturnFlow.do",
	// 删除
	deleteUrl:baseUrl + "fyhfDelete.do"
};
 ...
 * 恢复发起
 */
function fyhfAdd(title, code, ccrid, nodeid) {
	parent.$.modalDialog({
		title: "复议恢复发起",
		width: 900,
		height: 550,
		href: urls.fyhfAddInitUrl,
		queryParams: {
			ccrid: ccrid,
			nodeid: 10 
		},
		...
		buttons:[{
				...
				iconCls:"icon-save",
				handler:function() {
					submitForm(urls.fyhfAddSaveUrl, "fyhfAddForm", 10);
				}
			},{
				id: "sendBtn",
				disabled:true,
				...
				iconCls:"icon-redo",
				handler:function() {
					send(urls.fyhfAddSendUrl, ccrid, 10);
				}
			},{
				text:"关闭",
				iconCls:"icon-cancel",
...
function xzfyEditDialog(row) {
	...
	if (10 == nodeidtmp) {
		xzfyEdit("复议恢复发起", 900, 550, urls.fyhfAddInitUrl, row, "fyhfAddForm", urls.fyhfAddSaveUrl, urls.fyhfAddSendUrl);
	}
	else if (20 == nodeidtmp || 30 == nodeidtmp || 40 == nodeidtmp) {
		xzfyEdit("复议恢复审批", 900, 550, urls.fyhfAuditInitUrl, row, "fyhfAuditForm", urls.fyhfAuditSaveUrl, urls.fyhfAuditSendUrl);
	}
	else if (50 == nodeidtmp) {
		xzfyEdit("复议恢复决定", 900, 550, urls.fyhfDecideInitUrl, row, "fyhfDecideForm", urls.fyhfDecideSaveUrl, urls.fyhfDecideSendUrl);
	}
}
...
function isReadonlyByNodeid(nodeid){
	...
	var form = parent.$.modalDialog.handler.find('#fyhfAuditForm');
	if (20 == nodeid) { // 科室
...
function fyhfDelete() {
	...
	if(selectRow == null || selectRow.length == 0 || selectRow.length > 1){
		easyui_warn("请先选择一条恢复申请", null);
		return;
	}
	var row = xzfyDataGrid.datagrid("getSelections")[0];
	
	parent.$.messager.confirm("删除确认", "确认要删除该恢复申请？", function(r) {
		if (r) {
			$.post(urls.deleteUrl, {
				menuid:menuid,
				ccrid:row.ccrid 
