//请求路径
var baseUrl = contextpath + "aros/ajbg/fych/FychController/";
var urls = {
	// 查询复议程序案件列表
	gridUrl:baseUrl + "queryFychList.do",
	// 复议程序发起初始化
	fychAddInitUrl:baseUrl + "fychAddInit.do",
	// 复议程序发起保存
	fychAddSaveUrl:baseUrl + "fychAddSave.do",
	// 复议程序发起发送
	fychAddSendUrl:baseUrl + "fychAddFlow.do",
	// 复议程序审批编辑
	fychAuditInitUrl:baseUrl + "fychAuditInit.do",
	// 复议程序审批保存
	fychAuditSaveUrl:baseUrl + "fychAuditSave.do",
	// 复议程序审批发送
	fychAuditSendUrl:baseUrl + "fychAuditFlow.do",
	// 处理决定编辑
	fychDecideInitUrl:baseUrl + "fychDecideInit.do",
	// 处理决定保存
	fychDecideSaveUrl:baseUrl + "fychDecideSave.do",
	// 处理决定发送
	fychDecideSendUrl:baseUrl + "fychDecideFlow.do",
	// 回退
	backFlowPageUrl:baseUrl + "backFlowPage.do",
	// 回退
	returnUrl:baseUrl + "fychReturnFlow.do",
	// 删除
	deleteUrl:baseUrl + "fychDelete.do"
};
 ...
 * 程序发起
 */
function fychAdd(title, code, ccrid, nodeid) {
	parent.$.modalDialog({
		title: "复议程序发起",
		width: 900,
		height: 550,
		href: urls.fychAddInitUrl,
		queryParams: {
			ccrid: ccrid,
			nodeid: 10 
		},
		...
		buttons:[{
				...
				iconCls:"icon-save",
				handler:function() {
					submitForm(urls.fychAddSaveUrl, "fychAddForm", 10);
				}
			},{
				id: "sendBtn",
				disabled:true,
				...
				iconCls:"icon-redo",
				handler:function() {
					send(urls.fychAddSendUrl, ccrid, 10);
				}
			},{
				text:"关闭",
				iconCls:"icon-cancel",
...
function xzfyEditDialog(row) {
	...
	if (10 == nodeidtmp) {
		xzfyEdit("复议程序发起", 900, 550, urls.fychAddInitUrl, row, "fychAddForm", urls.fychAddSaveUrl, urls.fychAddSendUrl);
	}
	else if (20 == nodeidtmp || 30 == nodeidtmp || 40 == nodeidtmp) {
		xzfyEdit("复议程序审批", 900, 550, urls.fychAuditInitUrl, row, "fychAuditForm", urls.fychAuditSaveUrl, urls.fychAuditSendUrl);
	}
	else if (50 == nodeidtmp) {
		xzfyEdit("复议程序决定", 900, 550, urls.fychDecideInitUrl, row, "fychDecideForm", urls.fychDecideSaveUrl, urls.fychDecideSendUrl);
	}
}
...
function isReadonlyByNodeid(nodeid){
	...
	var form = parent.$.modalDialog.handler.find('#fychAuditForm');
	if (20 == nodeid) { // 科室
...
function fychDelete() {
	...
	if(selectRow == null || selectRow.length == 0 || selectRow.length > 1){
		easyui_warn("请先选择一条程序申请", null);
		return;
	}
	var row = xzfyDataGrid.datagrid("getSelections")[0];
	
	parent.$.messager.confirm("删除确认", "确认要删除该程序申请？", function(r) {
		if (r) {
			$.post(urls.deleteUrl, {
				menuid:menuid,
				ccrid:row.ccrid 
