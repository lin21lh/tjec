//请求路径
var baseUrl = contextpath + "aros/ajbg/zzfy/ZzfyController/";
var urls = {
	// 查询复议终止案件列表
	gridUrl:baseUrl + "queryZzfyList.do",
	// 复议终止发起初始化
	zzfyAddInitUrl:baseUrl + "zzfyAddInit.do",
	// 复议终止发起保存
	zzfyAddSaveUrl:baseUrl + "zzfyAddSave.do",
	// 复议终止发起发送
	zzfyAddSendUrl:baseUrl + "zzfyAddFlow.do",
	// 复议终止审批编辑
	zzfyAuditInitUrl:baseUrl + "zzfyAuditInit.do",
	// 复议终止审批保存
	zzfyAuditSaveUrl:baseUrl + "zzfyAuditSave.do",
	// 复议终止审批发送
	zzfyAuditSendUrl:baseUrl + "zzfyAuditFlow.do",
	// 处理决定编辑
	zzfyDecideInitUrl:baseUrl + "zzfyDecideInit.do",
	// 处理决定保存
	zzfyDecideSaveUrl:baseUrl + "zzfyDecideSave.do",
	// 处理决定发送
	zzfyDecideSendUrl:baseUrl + "zzfyDecideFlow.do",
	// 回退
	backFlowPageUrl:baseUrl + "backFlowPage.do",
	// 回退
	returnUrl:baseUrl + "zzfyReturnFlow.do",
	// 删除
	deleteUrl:baseUrl + "zzfyDelete.do"
};
 ...
 * 终止发起
 */
function zzfyAdd(title, code, ccrid, nodeid) {
	parent.$.modalDialog({
		title: "复议终止发起",
		width: 900,
		height: 550,
		href: urls.zzfyAddInitUrl,
		queryParams: {
			ccrid: ccrid,
			nodeid: 10 
		},
		onLoad:function() {
			...
			comboboxFuncByCondFilter(menuid, "endreason", "PUB_PROBASEINFO_END", "code", "name", mdDialog);       
		},
		onDestroy: function() {
			showReload();
		},
		buttons:[{
				...
				iconCls:"icon-save",
				handler:function() {
					submitForm(urls.zzfyAddSaveUrl, "zzfyAddForm", 10);
				}
			},{
				id: "sendBtn",
				disabled:true,
				...
				iconCls:"icon-redo",
				handler:function() {
					send(urls.zzfyAddSendUrl, ccrid, 10);
				}
			},{
				text:"关闭",
				iconCls:"icon-cancel",
...
function xzfyEditDialog(row) {
	...
	if (10 == nodeidtmp) {
		xzfyEdit("复议终止发起", 900, 550, urls.zzfyAddInitUrl, row, "zzfyAddForm", urls.zzfyAddSaveUrl, urls.zzfyAddSendUrl);
	}
	else if (20 == nodeidtmp || 30 == nodeidtmp || 40 == nodeidtmp) {
		xzfyEdit("复议终止审批", 900, 550, urls.zzfyAuditInitUrl, row, "zzfyAuditForm", urls.zzfyAuditSaveUrl, urls.zzfyAuditSendUrl);
	}
	else if (50 == nodeidtmp) {
		xzfyEdit("复议终止决定", 900, 550, urls.zzfyDecideInitUrl, row, "zzfyDecideForm", urls.zzfyDecideSaveUrl, urls.zzfyDecideSendUrl);
	}
}
...
function isReadonlyByNodeid(nodeid){
	...
	var form = parent.$.modalDialog.handler.find('#zzfyAuditForm');
	if (20 == nodeid) { // 科室
...
function zzfyDelete() {
	...
	if(selectRow == null || selectRow.length == 0 || selectRow.length > 1){
		easyui_warn("请先选择一条终止申请", null);
		return;
	}
	var row = xzfyDataGrid.datagrid("getSelections")[0];
	
	parent.$.messager.confirm("删除确认", "确认要删除该终止申请？", function(r) {
		if (r) {
			$.post(urls.deleteUrl, {
				menuid:menuid,
				ccrid:row.ccrid 
