//请求路径
var baseUrl = contextpath + "aros/ajbg/fyyq/FyyqController/";
var urls = {
	// 查询复议延期案件列表
	gridUrl:baseUrl + "queryFyyqList.do",
	// 复议延期发起初始化
	fyyqAddInitUrl:baseUrl + "fyyqAddInit.do",
	// 复议延期发起保存
	fyyqAddSaveUrl:baseUrl + "fyyqAddSave.do",
	// 复议延期发起发送
	fyyqAddSendUrl:baseUrl + "fyyqAddFlow.do",
	// 复议延期审批编辑
	fyyqAuditInitUrl:baseUrl + "fyyqAuditInit.do",
	// 复议延期审批保存
	fyyqAuditSaveUrl:baseUrl + "fyyqAuditSave.do",
	// 复议延期审批发送
	fyyqAuditSendUrl:baseUrl + "fyyqAuditFlow.do",
	// 处理决定编辑
	fyyqDecideInitUrl:baseUrl + "fyyqDecideInit.do",
	// 处理决定保存
	fyyqDecideSaveUrl:baseUrl + "fyyqDecideSave.do",
	// 处理决定发送
	fyyqDecideSendUrl:baseUrl + "fyyqDecideFlow.do",
	// 回退
	backFlowPageUrl:baseUrl + "backFlowPage.do",
	// 回退
	returnUrl:baseUrl + "fyyqReturnFlow.do",
	// 删除
	deleteUrl:baseUrl + "fyyqDelete.do"
};
 ...
 * 延期发起
 */
function fyyqAdd(title, code, ccrid, nodeid) {
	parent.$.modalDialog({
		title: "复议延期发起",
		width: 900,
		height: 550,
		href: urls.fyyqAddInitUrl,
		queryParams: {
			ccrid: ccrid,
			nodeid: 10 
		},
		...
		buttons:[{
				...
				iconCls:"icon-save",
				handler:function() {
					submitForm(urls.fyyqAddSaveUrl, "fyyqAddForm", 10);
				}
			},{
				id: "sendBtn",
				disabled:true,
				...
				iconCls:"icon-redo",
				handler:function() {
					send(urls.fyyqAddSendUrl, ccrid, 10);
				}
			},{
				text:"关闭",
				iconCls:"icon-cancel",
...
function xzfyEditDialog(row) {
	...
	if (10 == nodeidtmp) {
		xzfyEdit("复议延期发起", 900, 550, urls.fyyqAddInitUrl, row, "fyyqAddForm", urls.fyyqAddSaveUrl, urls.fyyqAddSendUrl);
	}
	else if (20 == nodeidtmp || 30 == nodeidtmp || 40 == nodeidtmp) {
		xzfyEdit("复议延期审批", 900, 550, urls.fyyqAuditInitUrl, row, "fyyqAuditForm", urls.fyyqAuditSaveUrl, urls.fyyqAuditSendUrl);
	}
	else if (50 == nodeidtmp) {
		xzfyEdit("复议延期决定", 900, 550, urls.fyyqDecideInitUrl, row, "fyyqDecideForm", urls.fyyqDecideSaveUrl, urls.fyyqDecideSendUrl);
	}
}
...
function isReadonlyByNodeid(nodeid){
	...
	var form = parent.$.modalDialog.handler.find('#fyyqAuditForm');
	if (20 == nodeid) { // 科室
...
function fyyqDelete() {
	...
	if(selectRow == null || selectRow.length == 0 || selectRow.length > 1){
		easyui_warn("请先选择一条延期申请", null);
		return;
	}
	var row = xzfyDataGrid.datagrid("getSelections")[0];
	
	parent.$.messager.confirm("删除确认", "确认要删除该延期申请？", function(r) {
		if (r) {
			$.post(urls.deleteUrl, {
				menuid:menuid,
				ccrid:row.ccrid 
