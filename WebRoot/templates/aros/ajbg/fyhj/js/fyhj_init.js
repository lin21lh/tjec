//请求路径
var baseUrl = contextpath + "aros/ajbg/fyhj/FyhjController/";
var urls = {
	// 查询复议和解案件列表
	gridUrl:baseUrl + "queryFyhjList.do",
	// 复议和解发起初始化
	fyhjAddInitUrl:baseUrl + "fyhjAddInit.do",
	// 复议和解发起保存
	fyhjAddSaveUrl:baseUrl + "fyhjAddSave.do",
	// 复议和解发起发送
	fyhjAddSendUrl:baseUrl + "fyhjAddFlow.do",
	// 复议和解审批编辑
	fyhjAuditInitUrl:baseUrl + "fyhjAuditInit.do",
	// 复议和解审批保存
	fyhjAuditSaveUrl:baseUrl + "fyhjAuditSave.do",
	// 复议和解审批发送
	fyhjAuditSendUrl:baseUrl + "fyhjAuditFlow.do",
	// 处理决定编辑
	fyhjDecideInitUrl:baseUrl + "fyhjDecideInit.do",
	// 处理决定保存
	fyhjDecideSaveUrl:baseUrl + "fyhjDecideSave.do",
	// 处理决定发送
	fyhjDecideSendUrl:baseUrl + "fyhjDecideFlow.do",
	// 回退
	backFlowPageUrl:baseUrl + "backFlowPage.do",
	// 回退
	returnUrl:baseUrl + "fyhjReturnFlow.do",
	// 删除
	deleteUrl:baseUrl + "fyhjDelete.do"
};
 ...
 * 和解发起
 */
function fyhjAdd(title, code, ccrid, nodeid) {
	parent.$.modalDialog({
		title: "复议和解发起",
		width: 900,
		height: 550,
		href: urls.fyhjAddInitUrl,
		queryParams: {
			ccrid: ccrid,
			nodeid: 10 
		},
		...
		buttons:[{
				...
				iconCls:"icon-save",
				handler:function() {
					submitForm(urls.fyhjAddSaveUrl, "fyhjAddForm", 10);
				}
			},{
				id: "sendBtn",
				disabled:true,
				...
				iconCls:"icon-redo",
				handler:function() {
					send(urls.fyhjAddSendUrl, ccrid, 10);
				}
			},{
				text:"关闭",
				iconCls:"icon-cancel",
...
function xzfyEditDialog(row) {
	...
	if (10 == nodeidtmp) {
		xzfyEdit("复议和解发起", 900, 550, urls.fyhjAddInitUrl, row, "fyhjAddForm", urls.fyhjAddSaveUrl, urls.fyhjAddSendUrl);
	}
	else if (20 == nodeidtmp || 30 == nodeidtmp || 40 == nodeidtmp) {
		xzfyEdit("复议和解审批", 900, 550, urls.fyhjAuditInitUrl, row, "fyhjAuditForm", urls.fyhjAuditSaveUrl, urls.fyhjAuditSendUrl);
	}
	else if (50 == nodeidtmp) {
		xzfyEdit("复议和解决定", 900, 550, urls.fyhjDecideInitUrl, row, "fyhjDecideForm", urls.fyhjDecideSaveUrl, urls.fyhjDecideSendUrl);
	}
}
...
function isReadonlyByNodeid(nodeid){
	...
	var form = parent.$.modalDialog.handler.find('#fyhjAuditForm');
	if (20 == nodeid) { // 科室
...
function fyhjDelete() {
	...
	if(selectRow == null || selectRow.length == 0 || selectRow.length > 1){
		easyui_warn("请先选择一条和解申请", null);
		return;
	}
	var row = xzfyDataGrid.datagrid("getSelections")[0];
	
	parent.$.messager.confirm("删除确认", "确认要删除该和解申请？", function(r) {
		if (r) {
			$.post(urls.deleteUrl, {
				menuid:menuid,
				ccrid:row.ccrid 
