...
var urls = {
	// 查询复议调解案件列表
	gridUrl:baseUrl + "queryFytjList.do",
	// 复议调解发起初始化
	fytjAddInitUrl:baseUrl + "fytjAddInit.do",
	// 复议调解发起保存
	fytjAddSaveUrl:baseUrl + "fytjAddSave.do",
	// 复议调解发起发送
	fytjAddSendUrl:baseUrl + "fytjAddFlow.do",
	// 复议调解审批编辑
	fytjAuditInitUrl:baseUrl + "fytjAuditInit.do",
	// 复议调解审批保存
	fytjAuditSaveUrl:baseUrl + "fytjAuditSave.do",
	// 复议调解审批发送
	fytjAuditSendUrl:baseUrl + "fytjAuditFlow.do",
	// 处理决定编辑
	fytjDecideInitUrl:baseUrl + "fytjDecideInit.do",
	// 处理决定保存
	fytjDecideSaveUrl:baseUrl + "fytjDecideSave.do",
	// 处理决定发送
	fytjDecideSendUrl:baseUrl + "fytjDecideFlow.do",
	// 回退
	backFlowPageUrl:baseUrl + "backFlowPage.do",
	// 回退
	returnUrl:baseUrl + "fytjReturnFlow.do",
	// 删除
	deleteUrl:baseUrl + "fytjDelete.do"
};
 ...
 * 调解发起
 */
function fytjAdd(title, code, ccrid, nodeid) {
	parent.$.modalDialog({
		title: "复议调解发起",
		width: 900,
		height: 550,
		href: urls.fytjAddInitUrl,
		queryParams: {
			ccrid: ccrid,
			nodeid: 10 
		},
		...
		buttons:[{
				...
				iconCls:"icon-save",
				handler:function() {
					submitForm(urls.fytjAddSaveUrl, "fytjAddForm", 10);
				}
			},{
				id: "sendBtn",
				disabled:true,
				...
				iconCls:"icon-redo",
				handler:function() {
					send(urls.fytjAddSendUrl, ccrid, 10);
				}
			},{
				text:"关闭",
				iconCls:"icon-cancel",
...
function xzfyEditDialog(row) {
	...
	if (10 == nodeidtmp) {
		xzfyEdit("复议调解发起", 900, 550, urls.fytjAddInitUrl, row, "fytjAddForm", urls.fytjAddSaveUrl, urls.fytjAddSendUrl);
	}
	else if (20 == nodeidtmp || 30 == nodeidtmp || 40 == nodeidtmp) {
		xzfyEdit("复议调解审批", 900, 550, urls.fytjAuditInitUrl, row, "fytjAuditForm", urls.fytjAuditSaveUrl, urls.fytjAuditSendUrl);
	}
	else if (50 == nodeidtmp) {
		xzfyEdit("复议调解决定", 900, 550, urls.fytjDecideInitUrl, row, "fytjDecideForm", urls.fytjDecideSaveUrl, urls.fytjDecideSendUrl);
	}
}
...
function isReadonlyByNodeid(nodeid){
	...
	var form = parent.$.modalDialog.handler.find('#fytjAuditForm');
	if (20 == nodeid) { // 科室
...
function fytjDelete() {
	...
	if(selectRow == null || selectRow.length == 0 || selectRow.length > 1){
		easyui_warn("请先选择一条调解申请", null);
		return;
	}
	var row = xzfyDataGrid.datagrid("getSelections")[0];
	
	parent.$.messager.confirm("删除确认", "确认要删除该调解申请？", function(r) {
		if (r) {
			$.post(urls.deleteUrl, {
				menuid:menuid,
				ccrid:row.ccrid 
