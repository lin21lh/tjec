...
var urls = {
	gridUrl:baseUrl + "queryAdmlitbaseinfoDivisionList.do",	
	editUrl:baseUrl + "admlitbaseinfoDivisionEditInit.do",
	editSaveUrl:baseUrl + "admlitbaseinfoDivisionSave.do",
	returnUrl:baseUrl + "admlitbaseinfoFlowForReturn.do",
	detailUrl:baseUrl + "admlitbaseinfoReqDetail.do",
	sendWFUrl:baseUrl + "admlitbaseinfoFlowForDivision.do",
	flowAddUrl:baseUrl + "updateOpttypeByCaseid.do" 
};

var panel_ctl_handles = [{
	panelname:"#ysajDivisionPanel", 		// 要折叠的面板id
	gridname:"#ysajDivisionDataGrid",  	// 刷新操作函数
...
$(function() {
	...
	//加载Grid数据
	loadYsajDivisionGrid(urls.gridUrl);
...
var ysajDivisionDataGrid;
var userDataGrid;

function showReload(){
	ysajDivisionDataGrid.datagrid("reload"); 
}

//加载可项目grid列表
function loadYsajDivisionGrid(url) {
	
	ysajDivisionDataGrid = $("#ysajDivisionDataGrid").datagrid({
		fit:true,
		stripe:true,
		singleSelect:true,
		rownumbers:true,
...
function ysajDivisionQuery(){
	...
	ysajDivisionDataGrid.datagrid("load", param);
}

/**
 * 查看行政应诉案件详细信息
 */
function ysajDivisionView(){
	
	var selectRow = ysajDivisionDataGrid.datagrid("getChecked");
	if(selectRow == null || selectRow.length == 0 || selectRow.length > 1){
		easyui_warn("请选择一条行政应诉案件", null);
		return;
	}
	
	var row = ysajDivisionDataGrid.datagrid("getSelections")[0];
...
function editYsajDivision(){
    ...
    var row = $('#ysajDivisionDataGrid').datagrid("getSelected"); 
    if (row){  
 ...
 * @param row
 */
function openYsajDivisionDialog(row){
	...
	parent.$.modalDialog({
		...
		onLoad:function() {
			...
			var f = parent.$.modalDialog.handler.find('#ysajDivisionForm');
			f.form("load", row);
			
			showFileDiv(mdDialog.find("#filetd"), false, "REQ", row.acaseid, "");
			showFileDiv(mdDialog.find("#divisionfiletd"), true, "DIVISION", row.acaseid, "");
		},
		buttons:[{
			id:"saveBtn",
			text:"保存",
			...
			handler:function() {
				...
				saveYsajDivisionForm(urls.editSaveUrl, "ysajDivisionForm", "");
			}
		},{
			id:"auditBtn",
			text:"发送",
 ...
 * @param form
 */
function saveYsajDivisionForm(url, form){
