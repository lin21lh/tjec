...
var urls = {
	gridUrl:baseUrl + "queryAdmlitbaseinfoAuditList.do",	
	editUrl:baseUrl + "admlitbaseinfoAuditEditInit.do",
	editSaveUrl:baseUrl + "admlitbaseinfoAuditSave.do",
	returnUrl:baseUrl + "admlitbaseinfoFlowForReturn.do",
	detailUrl:baseUrl + "admlitbaseinfoReqDetail.do",
	sendWFUrl:baseUrl + "admlitbaseinfoFlowForAudit.do",
	flowAddUrl:baseUrl + "updateOpttypeByCaseid.do" 
};

var panel_ctl_handles = [{
	panelname:"#ysajAuditPanel", 		// 要折叠的面板id
	gridname:"#ysajAuditDataGrid",  	// 刷新操作函数
...
$(function() {
	...
	//加载Grid数据
	loadYsajAuditGrid(urls.gridUrl);
...
var ysajAuditDataGrid;
var userDataGrid;

function showReload(){
	ysajAuditDataGrid.datagrid("reload"); 
}

//加载可项目grid列表
function loadYsajAuditGrid(url) {
	
	ysajAuditDataGrid = $("#ysajAuditDataGrid").datagrid({
		fit:true,
		stripe:true,
		singleSelect:true,
		rownumbers:true,
		...
		columns:[[ 
		  ...
		  {field:"lcasecode", title:"本机关案号", halign:"center", width:220, sortable:true},
		  {field:"rcasecode", title:"司法机关案号", halign:"center", width:220, sortable:true},
...
function ysajAuditQuery(){
	...
	ysajAuditDataGrid.datagrid("load", param);
}

/**
 * 查看行政应诉案件详细信息
 */
function ysajAuditView(){
	
	var selectRow = ysajAuditDataGrid.datagrid("getChecked");
	if(selectRow == null || selectRow.length == 0 || selectRow.length > 1){
		easyui_warn("请选择一条行政应诉案件", null);
		return;
	}
	
	var row = ysajAuditDataGrid.datagrid("getSelections")[0];
...
function editYsajAudit(){
    ...
    var row = $('#ysajAuditDataGrid').datagrid("getSelected"); 
    if (row){  
 ...
 * @param row
 */
function openYsajAuditDialog(row){
	...
	parent.$.modalDialog({
		...
		onLoad:function() {
			...
			comboboxFuncByCondFilter(menuid, "result", "AUDITRESULT", "code", "name", mdDialog);				//审批结果
			
			var f = parent.$.modalDialog.handler.find('#ysajAuditForm');
			f.form("load", row);
			
			showFileDiv(mdDialog.find("#filetd"), false, "REQ", row.acaseid, "");
			showFileDiv(mdDialog.find("#disposefiletd"), false, "DISPOSE", row.acaseid, "");
			showFileDiv(mdDialog.find("#auditfiletd"), true, "AUDIT", row.acaseid, "");
		},
		buttons:[{
			id:"saveBtn",
			text:"保存",
			...
			handler:function() {
				...
				saveYsajAuditForm(urls.editSaveUrl, "ysajAuditForm", "");
			}
		},{
			id:"auditBtn",
			text:"发送",
 ...
 * @param form
 */
function saveYsajAuditForm(url, form){
...
function sendAudit(row){
	...
	parent.$.messager.confirm("发送确认", "确认要发送该行政应诉案件？", function(r) {
		if (r) {
			$.post(urls.sendWFUrl, {
				menuid:menuid,
				activityId:activityId,
