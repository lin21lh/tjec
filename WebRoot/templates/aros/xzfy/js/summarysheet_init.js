...
var urls = {
	gridUrl:baseUrl + "querySummarySheetList.do",                 // 查询案件列表
	editUrl:baseUrl + "summaryEdit.do",                           // 打开备考表编辑、详情页面
	editSaveUrl:baseUrl + "summarySaveEdit.do",                       // 保存备考表信息
	sendWFUrl:baseUrl + "xzfySummaryFlow.do",               // 发送
	returnUrl:baseUrl + "xzfyReturn.do"                 // 回退
};

var panel_ctl_handles = [{
	panelname:"#summarySheetPanel", 	// 要折叠的面板id
	gridname:"#summarySheetDataGrid",  	// 刷新操作函数
...
function loadXzfyAcceptGrid(url) {
	...
	xzfyDataGrid = $("#summarySheetDataGrid").datagrid({
		fit:true,
		stripe:true,
		singleSelect:true,
		rownumbers:true,
...
function openSummaryDialog(row){
	...
	parent.$.modalDialog({
		...
		onLoad:function() {
			...
			var form = mdDialog.find("#summaryForm");
		},
		buttons:[{
			id:"saveBtn",
			text:"保存",
			...
			handler:function() {
				...
				saveSummaryForm(urls.editSaveUrl, "summaryForm");
			}
		},{
			text:"发送",
			iconCls:"icon-redo",
 ...
 * @param form
 */
function saveSummaryForm(url, form){
