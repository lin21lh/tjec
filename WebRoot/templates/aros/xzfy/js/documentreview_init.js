...
var urls = {
	gridUrl:baseUrl + "queryDocumentReviewList.do",                // 查询案件列表
	editUrl:baseUrl + "documentReviewEdit.do",                     // 打开处理页面
	editSaveUrl:baseUrl + "documentReviewSaveEdit.do",                 // 保存处理信息
	sendWFUrl:baseUrl + "xzfyDocReviewFlow.do",              // 发送
	returnUrl:baseUrl + "xzfyAcceptReturnEdit.do"                  // 回退
};

var panel_ctl_handles = [{
	panelname:"#documentReviewPanel", 	 // 要折叠的面板id
	gridname:"#documentReviewDataGrid",  // 刷新操作函数
...
$(function() {
	...
	$('#documentReviewPanel').panel('close');
...
function loadXzfyAcceptGrid(url) {
	...
	xzfyDataGrid = $("#documentReviewDataGrid").datagrid({
		fit:true,
		stripe:true,
		singleSelect:true,
		rownumbers:true,
 ...
 * @param row
 */
function openDocumentReviewDialog(row){
	parent.$.modalDialog({
		title:"回访单",
		width:900,
		height:600,
		...
		onLoad:function() {
			...
			var form = mdDialog.find("#documentReviewForm");
		},
		buttons:[{
			id:"saveBtn",
			text:"保存",
			...
			handler:function() {
				...
				saveDocumentReviewForm(urls.editSaveUrl, "documentReviewForm");
			}
		},{
			text:"发送",
			iconCls:"icon-redo",
 ...
 * @param form
 */
function saveDocumentReviewForm(url, form){
