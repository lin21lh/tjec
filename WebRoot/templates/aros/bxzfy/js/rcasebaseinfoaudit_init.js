...
var urls = {
	gridUrl:baseUrl + "queryRcasebaseinfoAuditList.do",	
	editUrl:baseUrl + "rcasebaseinfoAuditEditInit.do",
	editSaveUrl:baseUrl + "rcasebaseinfoAuditSave.do",
	returnUrl:baseUrl + "rcasebaseinfoFlowForReturn.do",
	detailUrl:baseUrl + "rcasebaseinfoReqDetail.do",
	sendWFUrl:baseUrl + "rcasebaseinfoFlowForAudit.do",
	flowAddUrl:baseUrl + "updateOpttypeByCaseid.do" 
};

var panel_ctl_handles = [{
	panelname:"#bxzfyAuditPanel", 		// 要折叠的面板id
	gridname:"#bxzfyAuditDataGrid",  	// 刷新操作函数
...
$(function() {
	...
	//加载Grid数据
	loadBxzfyAuditGrid(urls.gridUrl);
...
var bxzfyAuditDataGrid;
var userDataGrid;

function showReload(){
	bxzfyAuditDataGrid.datagrid("reload"); 
}

//加载可项目grid列表
function loadBxzfyAuditGrid(url) {
	
	bxzfyAuditDataGrid = $("#bxzfyAuditDataGrid").datagrid({
		fit:true,
		stripe:true,
		singleSelect:true,
		rownumbers:true,
...
function bxzfyAuditQuery(){
	...
	bxzfyAuditDataGrid.datagrid("load", param);
}

/**
 * 查看被复议案件详细信息
 */
function bxzfyAuditView(){
	
	var selectRow = bxzfyAuditDataGrid.datagrid("getChecked");
	if(selectRow == null || selectRow.length == 0 || selectRow.length > 1){
		easyui_warn("请选择一条被复议案件", null);
		return;
	}
	
	var row = bxzfyAuditDataGrid.datagrid("getSelections")[0];
...
function editBxzfyAudit(){
    ...
    var row = $('#bxzfyAuditDataGrid').datagrid("getSelected"); 
    if (row){  
 ...
 * @param row
 */
function openBxzfyAuditDialog(row){
	...
	parent.$.modalDialog({
		...
		onLoad:function() {
			...
			var f = parent.$.modalDialog.handler.find('#bxzfyAuditForm');
			f.form("load", row);
			
			showFileDiv(mdDialog.find("#filetd"), false, "BXZFYREQ", row.rcaseid, "");
			showFileDiv(mdDialog.find("#disposefiletd"), false, "DISPOSE", row.rcaseid, "");
			showFileDiv(mdDialog.find("#auditfiletd"), true, "AUDIT", row.rcaseid, "");
		},
		buttons:[{
			id:"saveBtn",
			text:"保存",
			...
			handler:function() {
				...
				saveBxzfyAuditForm(urls.editSaveUrl, "bxzfyAuditForm", "");
			}
		},{
			id:"auditBtn",
			text:"发送",
 ...
 * @param form
 */
function saveBxzfyAuditForm(url, form){
