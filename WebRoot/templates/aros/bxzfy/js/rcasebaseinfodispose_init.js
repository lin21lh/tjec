...
var urls = {
	gridUrl:baseUrl + "queryRcasebaseinfoDisposeList.do",	
	editUrl:baseUrl + "rcasebaseinfoDisposeEditInit.do",
	editSaveUrl:baseUrl + "rcasebaseinfoDisposeSave.do",
	returnUrl:baseUrl + "rcasebaseinfoFlowForReturn.do",
	detailUrl:baseUrl + "rcasebaseinfoReqDetail.do",
	sendWFUrl:baseUrl + "rcasebaseinfoFlowForDispose.do",
	flowAddUrl:baseUrl + "updateOpttypeByCaseid.do" 
};

var panel_ctl_handles = [{
	panelname:"#bxzfyDisposePanel", 		// 要折叠的面板id
	gridname:"#bxzfyDisposeDataGrid",  	// 刷新操作函数
...
$(function() {
	...
	//加载Grid数据
	loadBxzfyDisposeGrid(urls.gridUrl);
...
var bxzfyDisposeDataGrid;
var userDataGrid;

function showReload(){
	bxzfyDisposeDataGrid.datagrid("reload"); 
}

//加载可项目grid列表
function loadBxzfyDisposeGrid(url) {
	
	bxzfyDisposeDataGrid = $("#bxzfyDisposeDataGrid").datagrid({
		fit:true,
		stripe:true,
		singleSelect:true,
		rownumbers:true,
...
function bxzfyDisposeQuery(){
	...
	bxzfyDisposeDataGrid.datagrid("load", param);
}

/**
 * 查看被复议案件详细信息
 */
function bxzfyDisposeView(){
	
	var selectRow = bxzfyDisposeDataGrid.datagrid("getChecked");
	if(selectRow == null || selectRow.length == 0 || selectRow.length > 1){
		easyui_warn("请选择一条被复议案件", null);
		return;
	}
	
	var row = bxzfyDisposeDataGrid.datagrid("getSelections")[0];
...
function editBxzfyDispose(){
    ...
    var row = $('#bxzfyDisposeDataGrid').datagrid("getSelected"); 
    if (row){  
 ...
 * @param row
 */
function openBxzfyDisposeDialog(row){
	...
	parent.$.modalDialog({
		...
		onLoad:function() {
			...
			var f = parent.$.modalDialog.handler.find('#bxzfyDisposeForm');
			f.form("load", row);
			
			showFileDiv(mdDialog.find("#filetd"), false, "BXZFYREQ", row.rcaseid, "");
			showFileDiv(mdDialog.find("#disposefiletd"), true, "DISPOSE", row.rcaseid, "");
		},
		buttons:[{
			id:"saveBtn",
			text:"保存",
			...
			handler:function() {
				...
				saveBxzfyDisposeForm(urls.editSaveUrl, "bxzfyDisposeForm", "");
			}
		},{
			id:"auditBtn",
			text:"发送",
 ...
 * @param form
 */
function saveBxzfyDisposeForm(url, form){
