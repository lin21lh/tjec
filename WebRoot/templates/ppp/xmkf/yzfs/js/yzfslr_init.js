...
var urls = {
		//查询项目信息
		queryUrl : baseUrl+"yzfslrQuery.do"
		,addUrl : baseUrl+"yzfslrAdd.do"
		,addSaveUrl : baseUrl+"yzfslrAddSave.do"
		,deleteUrl : baseUrl+"yzfslrDelete.do"
		,detailUrl : baseUrl+"yzfslrDetail.do"
};
//列表信息
var projectDataGrid;
...
function yzfslrAdd(){
	...
	parent.$.modalDialog({
		...
		onLoad : function() {
			...
			comboboxFuncByCondFilter(menuid,"sftyyzfs", "SYS_TRUE_FALSE", "code", "name", mdDialog);
			comboboxFuncByCondFilter(menuid,"cgfs", "PURCHASE", "code", "name", mdDialog);
...
//			var sessionId = parent.$.modalDialog.handler.find('#sessionId').val();
			...
			//已录入备案信息
			if(row.yzfsid != null){
				showFileDiv(mdDialog.find("#filetd"),true,"YZFS",row.xmid,"");
			}
		},
		buttons : [ {
			text : "送审",
...
function yzfslrDelete(){
	...
	var row = projectDataGrid.datagrid("getSelections")[0];
	parent.$.messager.confirm("确认删除", "确认删除选中数据？", function(r) {
		if (r) {
			if(row.yzfsid == null){
...
//	parent.$.modalDialog.handler.find("#workflowflag").val(workflowflag);
	...
	if (!isValid) {
		parent.$.modalDialog.handler.find('#tabList').tabs('select', '项目运作备案');
		return;
	}
	form.form("submit",{
		url : url,
