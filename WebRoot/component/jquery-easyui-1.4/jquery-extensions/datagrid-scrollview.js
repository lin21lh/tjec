...
//			var style = styleValue ? 'style="' + styleValue + '"' : '';
	...
	onBeforeRender: function(target){
		...
		var opts = state.options;
		var dc = state.dc;
		if ($(dc.body2).scrollTop() > 0)
			this.last_scrollTop = $(dc.body2).scrollTop();
		var view = this;
		...
		function scrolling(){
			if (dc.body2.is(':empty')){
				reload.call(this);
				if (this.last_scrollTop && opts.positionScrollTop) {
					$(dc.body2).scrollTop(this.last_scrollTop);
					opts.positionScrollTop = false;
				}
